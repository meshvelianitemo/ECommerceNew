...
namespace ECommerceNew.Infrastructure.Repositories
{
    public class SmtpEmailService : IEmailService 
    {
        ...
        public async Task<string> SendVerificationEmailAsync(string destinationEmail)
        {
            var subject = "Email Verification Code";
            
            Random randomVerificationCode = new Random();
            var verificationCode = randomVerificationCode.Next(100000, 999999).ToString();
        ...
        public async Task<string> SendPasswordRecoveryEmailAsync(string destinationEmail)
        {
            var subject = "Password Recovery";
            Random randomVerificationCode = new Random();
            var passwordRecoveryCode = randomVerificationCode.Next(100000, 999999).ToString();

            var message = BuildPasswordRecoveryEmailHtml(passwordRecoveryCode);
            await SendEmailAsync(destinationEmail, subject, message);
