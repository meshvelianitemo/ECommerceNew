...
public interface IProductRepository
{
    ...
    Task AddImageUrlAsync(int productId, string Url, int userId, CancellationToken cancellationToken = default);
    Task<List<string>> ExtractImageUrl(int productId, CancellationToken cancellationToken = default);
    Task<bool> RemoveImageUrlAsync(int productId, string key, CancellationToken cancellationToken = default);
    Task<bool> AddToWishlist(int productId,int userId ,CancellationToken cancellationToken = default);
    ...
    Task<bool> AddToCart(int productId, int userId,int quantity, CancellationToken cancellationToken = default);
    Task<bool> RemoveFromCart(int productId , int userId ,CancellationToken cancellationToken = default);
    Task<PagedResult<CartItemDetailDto>> GetCartItemsForUserAsync(CartItemsQueryParameters queryParams, CancellationToken cancellationToken = default);
}
